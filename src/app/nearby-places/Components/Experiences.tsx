  ...
  return (
    <SectionWithContainer containerClassName="space-y-18">
      {items.map((item, index) => (
        <div key={index} className="grid md:grid-cols-2 grid-cols-1 items-center gap-8">
          <div className={`flex flex-col gap-6 ${index % 2 !== 0 ? "md:order-2" : ""}`}>
            ...
            <ul className="flex items-center box-shadow divide-x-[0.5px] divide-p2 py-3 w-fit rounded-sm">
              {item.details.slice(0, 2).map((detail, index) => (
                <li key={index} className="flex items-center gap-2.5 px-4">
                  {detail.icon}
